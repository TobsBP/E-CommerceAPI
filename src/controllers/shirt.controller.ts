...
import { type FastifyRequest, type FastifyReply, FastifyError } from 'fastify'
...
export async function getShirtController(
	request: FastifyRequest<{ Params: ShirtParams }>,
	reply: FastifyReply,
) {
	try {
...
export async function editShirtController(
  request: FastifyRequest<{ Body: ShirtParams }>,
	reply: FastifyReply,
 ) {
   try {
